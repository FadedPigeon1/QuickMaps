...
document.addEventListener("DOMContentLoaded", () => {
  ...
  modeButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      // If we have inputs, reload map immediately
  ...
  if (
    ...
    console.error(
      "GOOGLE_MAPS_API_KEY is not set. Create secrets.js from secrets.example.js and add your key.",
    );
    mapFrame.srcdoc =
      '<div style="display:flex;align-items:center;justify-content:center;height:100%;font-family:sans-serif;color:#666;text-align:center;padding:20px;">API key not configured.<br>See console for details.</div>';
    return;
  }
  ...
  function loadDirections(origin, destination) {
    ...
    const src = `https://www.google.com/maps/embed/v1/directions?key=${encodeURIComponent(GOOGLE_MAPS_API_KEY)}&origin=${encodedOrigin}&destination=${encodedDestination}&mode=${currentMode}`;
    mapFrame.src = src;
  }
